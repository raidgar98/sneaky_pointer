    ...
    bool get_flag(const unsigned int num) const
    {
        if(num == 0 || num >= (empty_bits + 1) || __pointer == nullptr)
            try{ throw std::invalid_argument("has to be in range <1; " +
                std::to_string((empty_bits)) + ">"); }catch(...){ throw; }
        int64_t mask = 1;
        mask <<= (sizeof(T*) * 8) - num;
...
template<typename T, uint8_t empty_bits>
std::ostream& operator<<(std::ostream& output, const sneaky_pointer<T, empty_bits>& src)
{
